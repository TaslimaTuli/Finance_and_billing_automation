...
const SignUpPage = () => {
      ...
      const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
          ...
          if (axios.isAxiosError(error)) {
            ...
            } else if (error.response?.status === 409 && error.response?.data?.message === 'Email is already taken') {
              setEmailExists(true);
  ...
  return (
    <div className="max-w-xl mx-auto mt-8 p-8 bg-slate-50 rounded-md shadow-md ">
        <h1 className="text-3xl font-bold mb-6 text-center">Finance and Billing Automation</h1>
      ...
      <form onSubmit={handleSubmit}>
        ...
        {usernameExists && (
        <p className="text-red-500 mt-2">Username is already taken. Please choose a different username.</p>
      )}
      {emailExists && (
        <p className="text-red-500 mt-2">Email is already taken. Please choose a different email.</p>
      )}
      {error && !usernameExists && !emailExists && <p className="text-red-500 mt-4">{error}</p>}
    {/* <br /> */}
